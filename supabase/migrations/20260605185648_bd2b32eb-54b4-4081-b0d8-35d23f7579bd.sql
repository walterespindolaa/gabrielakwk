...
-- 1) Convites públicos para formulários (link sem login)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.form_invites TO authenticated;
GRANT ALL ON public.form_invites TO service_role;

ALTER TABLE public.form_invites ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Staff manage invites" ON public.form_invites
  FOR ALL TO authenticated
  USING (is_staff()) WITH CHECK (is_staff());

CREATE INDEX idx_form_invites_cliente ON public.form_invites(cliente_id);
CREATE INDEX idx_form_invites_form ON public.form_invites(form_id);

-- 2) Seed do formulário de Pré-Consultoria CRIAR
INSERT INTO public.forms (id, title, description, stage, schema)
VALUES (
  '11111111-1111-1111-1111-111111111111',
  'Formulário de Pré-Consultoria — Método CRIAR',
  'Antes do nosso primeiro encontro, preciso te conhecer melhor. Reserve uns minutinhos e responda com calma — suas respostas são a base de tudo que vamos construir juntas.',
  'geral',
  $json${
    "intro": {
      "como_funciona": [
        ...
        {"titulo":"4 encontros online","texto":"Cada um com foco em uma etapa do Método CRIAR: Compreender, Reconhecer, Identificar e Ativar. Um por semana, pelo Zoom."},
        {"titulo":"Entrega final","texto":"Relatório mestre completo com posicionamento, persona, plano de 90 dias + 6 materiais de apoio para continuar criando com autonomia."},
